...
function Control(props: { pred: boolean, color: string, children?: Node, onClick: Function }) {
...
export default class StateRecorder<T> extends Component<Props<T>, State<T>> {
  ...
  render() {
    const { name = '', description = '', heading = true, stacked = false, fn } = this.props
    ...
    return (
      <Card name={name} description={description} heading={heading} stacked={stacked}>
        {fn(a)}
      </Card>
    )
  }
